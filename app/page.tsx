...
export default function Home() {
  ...
  useEffect(() => {
    const replay = async () => {
      ...
      if (!pending?.profile.syncSecret) return;
      try {
        const response = await fetch("/api/rota/sync", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${pending.profile.syncSecret}`,
          },
          body: JSON.stringify({
        ...
        if (response.ok) {
          ...
          setNotice(
            `Queued revision ${pending.revision.sequence} synced automatically.`,
          );
        }
      } catch {
        /* remain queued for the next online event */
      }
    };
  ...
  async function sync(download = false) {
    ...
    setSyncing(true);
    let pending: { profile: RotaProfile; revision: RotaRevision } | undefined;
    try {
      ...
      setNotice(
        `${e instanceof Error ? e.message : "Sync failed"} Saved safely and queued for retry.`,
      );
    } finally {
      setSyncing(false);
    }
    ...
    <main className="mx-auto min-h-screen max-w-[1500px] px-3 pb-16 pt-4 sm:px-6 lg:px-8">
      ...
      <section className="glass mt-5 flex flex-col gap-4 rounded-3xl p-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            onClick={async () => {
              const { downloadRotaPdf } = await import("@/lib/pdf-export");
              await downloadRotaPdf(result.metadata, schedule);
            }}
            className="flex items-center gap-2 rounded-xl border border-sky-400/25 bg-sky-400/10 px-4 py-3 text-sm font-semibold text-sky-200"
          >
            <FileDown size={16} />
            PDF report
          </button>
