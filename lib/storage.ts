...
export async function savePendingSync(value: PendingSync) {
  return (await db()).put("drafts", value, "pending-sync");
}
export async function getPendingSync() {
  return (await db()).get("drafts", "pending-sync") as Promise<
    PendingSync | undefined
  >;
}
export async function clearPendingSync() {
  return (await db()).delete("drafts", "pending-sync");
}
